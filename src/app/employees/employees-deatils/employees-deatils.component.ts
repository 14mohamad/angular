import { Component, OnInit } from '@angular/core';
import Employee from '../Employee';
import { EmployeesService } from '../employees.service';

@Component({
  selector: 'app-employees-deatils',
  templateUrl: './employees-deatils.component.html',
  styleUrls: ['./employees-deatils.component.css'],
})
export class EmployeesDeatilsComponent implements OnInit {
  employees: Employee[] = [];
  constructor(private service: EmployeesService) {}

  ngOnInit(): void {
    this.getEmployees();
  }
  getEmployees(): void {
    this.service
      .getEmployees()
      .subscribe((employee) => (this.employees = employee));
  }

  getEmployeeById(id: string): Employee | undefined {
    let result: Employee | undefined;
    this.service.getEmployeeById(id).subscribe((res) => (result = res));
    return result;
  }

  getRowColor(salary: number): string {
    return salary > 3000 ? 'CornflowerBlue' : 'IndianRed';
  }
}
